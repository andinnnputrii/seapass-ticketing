    ...
    <div id="app-shell">
        ...
        <aside class="sidebar bg-white border-r border-slate-200">
            <div class="h-16 flex items-center gap-3 px-4 border-b border-slate-200">
                <div class="w-8 h-8 rounded-md bg-brand-700 flex items-center justify-center text-white font-semibold">S
                </div>
            ...
            @php
                $items = $menuItems ?? [
                    ...
                    ['label' => 'Laporan & Analitik', 'icon' => 'bar-chart-2', 'route' => route('admin.reports.index')],
                    ['label' => 'Pengguna & Akses', 'icon' => 'shield', 'route' => route('admin.users.index')],
            ...
            <nav class="p-3">
                <ul class="space-y-1">
                    @foreach ($items as $item)
                        @php
                            // Cek apakah menu aktif (support untuk sub-routes seperti transactions.index, transactions.show)
                            $active =
                                $current === $item['route'] ||
                                (request()->routeIs('admin.transactions.*') &&
                                    $item['label'] === 'Transaksi & Refund') ||
                                (request()->routeIs('admin.refunds.*') && $item['label'] === 'Transaksi & Refund');
                        @endphp
                        <li>
                            <a href="{{ $item['route'] }}"
        ...
        <div class="min-h-screen flex flex-col">
            ...
            <header class="h-16 bg-brand-800 text-white px-4 lg:px-6 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button class="p-2 rounded-md hover:bg-brand-700" aria-label="Notifikasi"><i
                            data-feather="bell"></i></button>
                    <div class="flex items-center gap-2">
                        ...
                        <div class="w-8 h-8 rounded-full bg-white/20 grid place-items-center">
                            {{ strtoupper(substr(session('admin_username', 'AD'), 0, 2)) }}
                        </div>
                    </div>
                </div>
            </header>
